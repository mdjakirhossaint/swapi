...
namespace SoowGoodWeb.Controllers
{
    ...
    public class CommonController : AbpController
    {
        ...
        public async Task<IActionResult> FileUploadDocuments()
        {
            ...
            try
            {
                var idStrs = Request.Form["entityId"].ToString();
                long attchmentId = 0;
                ...
                if (files.Count > 0)
                {
                    ...
                    foreach (var file in files)
                    {
                        ...
                        {
                            if (attachmentType == AttachmentType.ProfilePicture.ToString() || attachmentType == AttachmentType.DoctIdentityDoc.ToString()||attachmentType== AttachmentType.DoctorSign.ToString())
                            {
