...
namespace SoowGoodWeb.Services
{
    public class DashboardService : SoowGoodWebAppService
    {
        ...
        public async Task<DashboardDto> GetDashboadDataForDoctorAsync(long doctorid)
        {
            ...
            result.todayConsulted= appointments.Where(a=>a.AppointmentStatus == AppointmentStatus.Completed&& a.AppointmentDate?.Date == DateTime.Today).Count();
            var todayIncome = appointments
          .Where(a => a.AppointmentDate?.Date == DateTime.Today)
          .Sum(a => a.DoctorFee ?? 0);

            result.todayFeeAmount = todayIncome;
            return result;
        }
        ...
        public async Task<IActionResult> GetDashboardAppointmentListForDoctorAsync(long doctorId, string day, FilterModel filterModel)
        {
            CultureInfo provider = CultureInfo.InvariantCulture;
            ...
            try
            {
                 ...
                 if (day == "confirmed")
                {
                 ...
                 if (day == "completed")
                {
                    appointments = allAppointments.Where(p => p.DoctorProfileId == doctorId && (p.AppointmentStatus == AppointmentStatus.Completed) && ((p.ConsultancyType == ConsultancyType.Instant) || (p.ConsultancyType == ConsultancyType.Online))).ToList();
                }

                 if (day == "today")
                {
                    appointments = allAppointments.Where(p => p.DoctorProfileId == doctorId && (p.AppointmentStatus == AppointmentStatus.Confirmed||p.AppointmentStatus == AppointmentStatus.Completed) && p.AppointmentDate.HasValue &&
        p.AppointmentDate.Value.Date == DateTime.Today &&(p.ConsultancyType==ConsultancyType.Instant||p.ConsultancyType == ConsultancyType.Online)).ToList();
                }

                if (!appointments.Any())
                {
                    return new OkObjectResult(new { result = new List<AppointmentDto>(), count = 0 });
                }
                result = new List<AppointmentDto>();
    ...
    .ToList();
        ...
        public async Task<List<AppointmentDto>> GetDashboardAppointmentListForPatientAsync(long patientId, string role, string day, FilterModel filterModel)
        {
            CultureInfo provider = CultureInfo.InvariantCulture;
            var result = new List<AppointmentDto>();
            try
