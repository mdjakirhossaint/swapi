...
namespace SoowGoodWeb.Services
{
    public class DoctorScheduleService : SoowGoodWebAppService, IDoctorScheduleService
    {
        ...
        public async Task<List<DoctorScheduleDto>?> GetListByDoctorIdListAsync(long doctorId)
        {
            ...
            foreach (var schedule in item)
            {
                result.Add(new DoctorScheduleDto()
                {
                    ...
                    Chamber = schedule.DoctorChamberId > 0 ? schedule.DoctorChamber?.ChamberName : schedule.ConsultancyType.HasValue
        ...
        public async Task<List<DoctorScheduleDto>?> GetScheduleListByDoctorIdAsync(long doctorId)
        {
            List<DoctorScheduleDto>? result = null;
            var allSchedule =
                await _doctorScheduleRepository.WithDetailsAsync(d => d.DoctorProfile, c => c.DoctorChamber, c=>c.DoctorScheduleDaySession);
            ...
            foreach (var schedule in item)
            {
                ...
                if (schedule.DoctorChamberId > 0)
                {
                    var chamber = await _doctorChamberRepository.FirstAsync(c => c.Id == schedule.DoctorChamberId);
                ...
                result.Add(new DoctorScheduleDto()
                {
                    ...
                    ScheduleTypeName = schedule.ScheduleType.ToString(),
                    Chamber=chamberName,




                    DoctorScheduleDaySession = schedule.DoctorScheduleDaySession?.Select(ds => new DoctorScheduleDaySessionDto
                    {
                        Id = ds.Id,
                        DoctorScheduleId = ds.DoctorScheduleId,
                        DoctorScheduleName = schedule.ScheduleName,
                        ScheduleDayofWeek = ds.ScheduleDayofWeek,
                        StartTime = ds.StartTime,
                        EndTime = ds.EndTime,
                        NoOfPatients = ds.NoOfPatients,
        ...
        public async Task<List<DoctorScheduleDto>?> GetDetailsScheduleListByDoctorChamberIdAsync(long doctorId,long chamberId)
        {
            List<DoctorScheduleDto>? result = null;
            var allSchedule =
                await _doctorScheduleRepository.WithDetailsAsync(d => d.DoctorProfile, s => s.DoctorScheduleDaySession, c => c.DoctorChamber, f => f.DoctorFeesSetup);
            var item = allSchedule.Where(s => s.DoctorProfileId == doctorId & s.DoctorChamberId==chamberId).ToList();
            if (!item.Any())
            {
                return result; // ObjectMapper.Map<List<DoctorSchedule>, List<DoctorScheduleDto>>(schedules);
            }
